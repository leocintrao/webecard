...
require_once "source/PagSeguroLibrary/PagSeguroLibrary.php"; 
	...
	//$transaction->getReference(); // fucking works
	
	// *** codificar, qq um pode mandar POST com status 3...
	
	if ( $status->getValue() == 3 ) { // 3 = PAID // 1 = WAITING_PAYMENT -> used to try and see if it approves no problem
	...
	$referencia = $transaction->getReference(); // fucking works
		...
		if (mysqli_num_rows($result) > 0) {
			...
			// adiciona creditos					
			if (!mysqli_query($link, "INSERT INTO creditos (user_id, tipo_cred, valor, saldo, referencia)

								VALUES ('$user_id', 'p', '$valor', '$saldo', '$referencia') ")) 
		...
		$message = "
			...
			echo "Done!";
				...
				$to = "info@webecard.net";
				$from = "info@webecard.net";
				$subject = "$referencia nao encontrado";
